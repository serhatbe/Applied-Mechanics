...
\begin{tikzpicture}[scale=0.25]
    ...
    \def\mA{25}    % Magnitude of vector A
    \def\angleA{60} % Angle of vector A (degrees)

    \def\mB{15}    % Magnitude of vector B
    \def\angleB{330} % Angle of vector B (degrees)
    ...
    \draw[->, thick, blue] (O) -- (A) node[midway, left=9pt] {$\vec{v_1} (\mA\,m/s, \angleA^\circ)$};
    \draw[->, thick, red] (A) -- ++(B) node[midway, right=5pt] {$\vec{v_2} (\mB\,m/s, \angleB^\circ)$};
    %\draw[->, thick, green] (A) ++ (B) -- ++(C) node[midway, above=5pt] {$\vec{C} (\mC, \angleC^\circ)$};
    
    % Draw the resultant vector R from the origin
    \draw[->, thick, purple] (O) -- (R) node[midway, right=15pt] {$|\vec{R}| = \mR$\,m/s, $\theta_R = \angleR^\circ$};
\end{tikzpicture}
