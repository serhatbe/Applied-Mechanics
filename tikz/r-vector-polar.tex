...
\begin{tikzpicture}
    ...
    \def\mA{10}    % Magnitude of vector A
    \def\angleA{30} % Angle of vector A (degrees)

    \def\mB{20}    % Magnitude of vector B
    \def\angleB{90} % Angle of vector B (degrees)

    \def\mC{15}    % Magnitude of vector C
    \def\angleC{195} % Angle of vector C (degrees)
    ...
    \draw[->, thick, red] (A) -- ++(B) node[midway, right] {$\vec{B} (\mB, \angleB^\circ)$};
    \draw[->, thick, green] (A) ++ (B) -- ++(C) node[midway, above left] {$\vec{C} (\mC, \angleC^\circ)$};
    
    % Draw the resultant vector R from the origin
    \draw[->, thick, purple] (O) -- (R) node[midway, left=5pt] {$|\vec{R}| = \mR$, $\theta_R = \angleR^\circ$};
\end{tikzpicture}
